...
def process_nhai_data(df: pd.DataFrame) -> List[Dict[str, Any]]:
    ...
    # Skip the first two rows (headers) if they exist
    if len(df) > 2:
        data_rows = df.iloc[2:]
    else:
        data_rows = df
...
def extract_lane_coordinates(row: pd.Series) -> List[Dict[str, Any]]:
    ...
    # Lane coordinate mapping based on the CSV structure
    lane_mapping = [
...
def extract_measurements(row: pd.Series) -> List[Dict[str, Any]]:
    ...
    try:
        # Roughness BI measurements (columns 39-46)
        ...
        # Rut Depth measurements (columns 48-55)
        ...
        # Cracking measurements (columns 57-64)
        ...
        # Ravelling measurements (columns 66-73)
...
def aggregate_data_by_location(data: List[Dict[str, Any]], radius_km: float = 1.0) -> List[Dict[str, Any]]:
    ...
    for i, point in enumerate(data):
        ...
        if len(nearby_points) > 1:
            # Use the point with highest severity as the representative
